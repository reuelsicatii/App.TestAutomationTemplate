...
@CucumberOptions(
        ...
        plugin = {"com.cucumber.listener.ExtentCucumberFormatter:target/R1_SrsSmokeTestSupportMenuReport.html","rerun:target/SupportMenuRerun2.txt"}
   

)


public class R2_SrsSmokeTestSupportMenu extends WEBHelper{
	@BeforeClass
	...
	{		
		...
		BufferedReader br = new BufferedReader(new FileReader("target/SupportMenuRerun.txt"));
		...
		if (br.readLine() == null){
			...
			Mail.SendReport("R1_SrsSmokeTestSupportMenuReport.html", GetApplication() + GetTestEnv(), "[SMOKE TEST - RERUN]: " + GetApplication() + GetTestEnv() + " (Support Menu:"+testStatus+") - ");
